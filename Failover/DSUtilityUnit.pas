...
uses
  SysUtils, Classes, DBXJSON, DBXPlatform, Generics.Collections,
  DataBkr, System.Json, DataSnap.DSProviderDataModuleAdapter, Datasnap.DSServer,
  Datasnap.DSAuth;
...
uses
  DBXDatasnap, DBXCommon, Windows;

type
  TParamSetup = reference to procedure(Params: TDBXParameterList);

procedure ExecuteRemote(AHostName, AProtocol, APort: string; AClassName,
  AMethodName: string; ParamSetup: TParamSetup; ParamCheckup: TParamSetup);
var
...
begin
  ...
  begin
    ...
    answer.AddPair(TJSONPair.Create(TJSONString.Create('Received'), TJSONTrue.Create));
...
function TDSUtilityMethods.strings(str1: ansistring; var str2: ansistring; out
  str3: ansistring): ansistring;
begin
  Result := str1;
...
begin
  ...
  begin
    ...
    raise Exception.Create('Object already exists with key ' + Key +
      '. Use update or use a different key.');
end;
